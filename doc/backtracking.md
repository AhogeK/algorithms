...
* [回溯算法](#回溯算法)
  ...
  * [数独问题](#数独问题)
    ...
    * [完成「力扣」第 401 题：二进制手表](#完成力扣第-401-题二进制手表)
      ...
      * [代码实现](#代码实现-27)
      * [复杂度分析](#复杂度分析-41)
    * [Dancing Links（跳舞链表）](#dancing-links跳舞链表)
      * [精确覆盖是什么](#精确覆盖是什么)
      * [Algorithm X 是什么（与 DLX 的关系）](#algorithm-x-是什么与-dlx-的关系)
      * [DLX 的数据结构长什么样](#dlx-的数据结构长什么样)
      * [cover / uncover 为什么能快（核心技巧）](#cover--uncover-为什么能快核心技巧)
      * [为什么叫 “Dancing Links（跳舞链表）”](#为什么叫-dancing-links跳舞链表)
      * [典型递归框架（DLX + Algorithm X）](#典型递归框架dlx--algorithm-x)
      * [复杂度直觉（为什么它快）](#复杂度直觉为什么它快)
      * [用 DLX 解数独的建模（最常见例子）](#用-dlx-解数独的建模最常见例子)
      * [实战细节与坑点](#实战细节与坑点)
<!-- TOC -->
...
> **Flood Fill (泛洪填充)** 是一种经典的算法，主要用于在多维数组（通常是二维网格/图像）中确定连接的区域。
> 
> 最直观的例子就是**画图软件（MS Paint）中的“油漆桶”工具** 🎨。当你点击图像中的某个像素时，所有与该像素颜色相同且相连的区域都会被填充为新的颜色。
> 
> 虽然它通常被归类为 **DFS（深度优先搜索）** 或 **BFS（广度优先搜索）** 的应用，但在算法竞赛和 LeetCode 中，它经常作为**回溯算法**（或者更准确地说是图的遍历）的基础练习题出现。

### 核心原理与回溯的关系

Flood Fill 本质上是在在一个**隐式图（Implicit Graph）**上寻找**连通分量（Connected Component）**。

1. **它是回溯算法吗？**

    严格来说，标准的 Flood Fill 属于 **DFS（深度优先搜索）**。

    * **纯回溯 (Backtracking)**：通常包含“**做选择** $\to$ **递归** $\to$ **撤销选择**”的步骤。主要用于寻找所有解或特定路径。
    * **Flood Fill**：通常包含“**做选择（标记/染色）** $\to$ **递归**”的步骤，但往往**不需要撤销选择**。因为我们的目的就是把整个区域染成新颜色，或者标记为已访问，不需要回头。

    **但是**，在某些变种问题（如“单词搜索”）中，我们需要在网格中寻找路径，这时就会结合 Flood Fill 的遍历方式和回溯的“撤销”机制（标记为 visited，递归回来后取消 visited）。

2. **算法流程**

    假设我们在一个二维矩阵中，从坐标 $(r, c)$ 开始：

    1. **检查边界**：如果 $(r, c)$ 超出矩阵范围，停止。
...
> N 皇后问题：在一个 $n \times n$ 的棋盘上放置 $n$ 个皇后，使任意两个皇后都不互相攻击（皇后能沿行、列、两条对角线攻击）。常见目标是“输出所有可行棋盘布局”或“统计可行解的数量”。

### 约束条件怎么检查

通常按“每行放 1 个皇后”的方式构造解，这样天然避免了同行冲突，只需保证：

* 不同列：列号不能重复
* 主对角线（左上到右下）：同一条对角线满足 $r - c$ 相等
* 副对角线（右上到左下）：同一条对角线满足 $r + c$ 相等

所以可以用三个集合/布尔数组来标记是否被占用：`col[c]`、`diag1[r-c]`、`diag2[r+c]`。

### 经典解法：回溯（DFS）

从第 0 行开始，依次尝试在当前行的每一列放皇后：

* 如果该列、两条对角线都没被占用，就放置并打标记
* 递归到下一行
* 回溯时撤销放置与标记

这类问题本质是枚举 + 剪枝，回溯是最直观的做法。

### 复杂度与性质

搜索树上界接近 $n!$（每行选一个不同列），加上对角线剪枝后实际会小很多，但整体仍是指数级问题；常见表述是时间复杂度约为 $\mathcal{O}(n!)$ 量级，空间为递归栈和标记数组的 $\mathcal{O}(n)$。
...
> Dancing Links（跳舞链表，常简称 DLX）是 Donald Knuth 为实现“Algorithm X（算法 X）”而设计的一种数据结构技巧，用来高效求解 
> 精确覆盖（Exact Cover） 问题。它的核心不是“更聪明地剪枝”，而是把“选择/撤销选择”这两类操作做到了接近常数开销，从而让深度回溯在大量撤销时依然很快。

#### 精确覆盖是什么

给定一个由 $0/1$ 组成的矩阵，目标是从若干行中选出一个子集，使得：

* 每一列中恰好有一行被选中（覆盖一次）
* 也就是每一列的覆盖次数等于 $1$

用集合语言描述：每一行对应一个集合（包含若干列元素），要求选一些集合，使得每个元素被覆盖恰好一次。
...
#### Algorithm X 是什么（与 DLX 的关系）

Algorithm X 是一种“递归 + 回溯”的精确覆盖求解算法。它每一步做的事情非常直白：

1. 从还没覆盖的列里选一列（常用启发：选 1 的数量最少的列）
2. 尝试选择该列里为 1 的某一行
3. 选择该行后，会“覆盖”该行涉及到的所有列，并删除与这些列冲突的行
4. 递归继续；失败则撤销（“取消覆盖”）并换下一行

难点在于：如果用普通数组/位集来“删行删列”，撤销会很麻烦（要恢复很多状态）。DLX 用“可逆的链表删除”把 cover/uncover 变成了非常快的指针操作，这就是“跳舞”的由来：删掉的节点像跳舞一样又能原样回来。

#### DLX 的数据结构长什么样

把矩阵里所有的 $1$ 都变成一个“节点（Node）”，每个节点同时处在两条循环双向链表里：

* 同一行：用 `L/R` 串起来（行内横向循环双链）
* 同一列：用 `U/D` 串起来（列内纵向循环双链）

另外每一列还有一个“列头（Column Header）”，列头也在一条“列头链表”里（横向），方便快速遍历当前还存在的列。列头通常还维护一个 `size`，表示该列目前还有多少个节点（也就是还有多少可选行能覆盖它）。
...
可以把它想成一个“稀疏矩阵的十字链表 + 可逆删除”。

#### cover / uncover 为什么能快（核心技巧）

DLX 的关键操作只有两个：`cover(col)` 与 `uncover(col)`。它们都只做“链表摘除/挂回”，不需要拷贝矩阵，也不需要额外保存一大堆快照，因为链表指针本身就保留了恢复信息。

`cover(col)`：**选择覆盖某列**

当你决定“要覆盖某列 col”，你要做两层删除：

* 先把列头 `col` 从“列头链表”里摘掉（这一列不再需要被覆盖）
* 对于 `col` 列里的每个节点 `r`（每个 `r` 对应一条候选行）：
    * 遍历 `r` 这一行上的其它节点 `j`
    * 把 `j` 从它所属的列链表中摘掉（因为选了这行后，任何与这行共享列的其它行都冲突，要删除）

这相当于实现了 Algorithm X 中的“删除所有冲突行/列”。

`uncover(col)`：**撤销覆盖某列**

撤销时严格按 cover 的逆序把节点挂回去：

* 对 `col` 列里的节点按相反方向遍历
* 对每个行节点 `r` 的行内节点 `j` 按相反方向遍历，把 `j` 重新挂回其列链表
* 最后把列头 `col` 挂回列头链表

因为每次“摘除”并没有破坏节点自身的 `L/R/U/D` 信息，只是改了邻居指针，所以恢复是局部且可逆的。

#### 为什么叫 “Dancing Links（跳舞链表）”

> 直观理解：在回溯搜索中，节点会不断被“拿出链表”（`cover` 时摘除），失败后又被“放回链表”（`uncover` 时恢复）。
> 这些节点在链表里进进出出、来回跳动，就像在跳舞一样，因此 Knuth 称之为 Dancing Links。

#### 典型递归框架（DLX + Algorithm X）

算法的骨架是：

* 若所有列都被覆盖（列头链表为空） $\rightarrow$ 找到一个解
* 否则选一个列 `c`（通常选 `size` 最小的列，强剪枝）
* `cover(c)`
* 对 `c` 列中的每一行节点 `r`：
    * 把 `r` 对应的“行”加入解集
    * 对 `r` 行内的每个节点 `j`：`cover(j.C)`
    * 递归
    * 回溯：对 `r` 行内节点反向 `uncover(j.C)`，移除该行
* `uncover(c)`

$$\begin{aligned} &\textbf{Search}():\\\ &\quad \text{if } \text{header.R} = \text{header} \text{ then output solution and return}\\\ &\quad c \leftarrow \text{ChooseColumnMinSize}()\\\ &\quad \text{Cover}(c)\\\ &\quad \text{for } r \leftarrow c.D; \ r \ne c; \ r \leftarrow r.D \text{ do}\\\ &\qquad \text{push}(r)\\\ &\qquad \text{for } j \leftarrow r.R; \ j \ne r; \ j \leftarrow j.R \text{ do Cover}(j.C)\\\ &\qquad \textbf{Search}()\\\ &\qquad \text{for } j \leftarrow r.L; \ j \ne r; \ j \leftarrow j.L \text{ do Uncover}(j.C)\\\ &\qquad \text{pop}(r)\\\ &\quad \text{Uncover}(c) \end{aligned}$$

这里 `ChooseColumnMinSize()` 就是常说的 “S 启发式”：选当前 1 最少的列，尽早失败，减少分支。

#### 复杂度直觉（为什么它快）

精确覆盖本质是 NP 完全问题，最坏复杂度仍然是指数级，DLX 不能改变理论上界；它的优势在于把回溯里的“状态维护”成本压到很低。

可以这样理解成本构成：

* 选择一行所引起的删点/恢复点次数，与该行涉及的 1 的数量及其冲突规模有关
* DLX 的 cover/uncover 仅修改常数个指针（加上必要的遍历），不需要额外的矩阵拷贝
* 选最小列相当于让分支因子接近最小，使搜索树显著变小

因此在数独、拼图等典型场景中，DLX 往往比“位集 + 回溯 + 手动恢复”更稳定、更快（尤其当撤销操作非常频繁时）。

#### 用 DLX 解数独的建模（最常见例子）

以 $9 \times 9$ 数独为例，每个候选填数（某格填某数）对应矩阵的一行，一共 $9 \times 9 \times 9 = 729$ 行。

它需要满足 4 类约束（也就是 4 类列，一共 $324$ 列）：

* 格子约束：每个格子必须填一个数（ $81$ 列）
* 行约束：每行每个数出现一次（ $81$ 列）
* 列约束：每列每个数出现一次（ $81$ 列）
* 宫约束：每宫每个数出现一次（ $81$ 列）

所以精确覆盖矩阵规模是 $729 \times 324$ 的稀疏 $0/1$ 矩阵；DLX 恰好适合这种“稀疏、撤销频繁”的场景。

#### 实战细节与坑点

* `uncover` 必须严格按 `cover` 的逆序恢复，否则链表会断
* 列大小 `size` 要在摘除/恢复节点时同步增减，否则“选最小列”会失效
* 为了快，一般用数组模拟节点池（下标当指针），避免频繁 new 对象
* 解的输出：存的是“选中的行”，再由行 $\rightarrow$ 具体含义（例如数独的格子与数字）进行反解析
