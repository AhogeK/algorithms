...
* [动态规划（上）](#动态规划上)
  ...
  * [理解「无后效性」](#理解无后效性)
    ...
    * [完成「力扣」第 120 题：三角形最小路径和](#完成力扣第-120-题三角形最小路径和)
      ...
      * [代码实现](#代码实现-9)
      * [复杂度分析](#复杂度分析-9)
    * [完成「力扣」第 63 题：不同路径 II](#完成力扣第-63-题不同路径-ii)
      * [算法思路](#算法思路-10)
      * [代码实现](#代码实现-10)
      * [复杂度分析](#复杂度分析-10)
    * [完成「力扣」第 64 题：最小路径和](#完成力扣第-64-题最小路径和)
      * [算法思路](#算法思路-11)
      * [代码实现](#代码实现-11)
      * [复杂度分析](#复杂度分析-11)
    * [完成「力扣」第 121 题：买卖股票的最佳时机](#完成力扣第-121-题买卖股票的最佳时机)
      * [算法思路](#算法思路-12)
      * [代码实现](#代码实现-12)
      * [复杂度分析](#复杂度分析-12)
    * [完成「力扣」第 122 题：买卖股票的最佳时机 II](#完成力扣第-122-题买卖股票的最佳时机-ii)
      * [算法思路](#算法思路-13)
      * [代码实现](#代码实现-13)
      * [复杂度分析](#复杂度分析-13)
    * [完成「力扣」第 123 题：买卖股票的最佳时机 III](#完成力扣第-123-题买卖股票的最佳时机-iii)
      ...
      * [代码实现](#代码实现-14)
      * [复杂度分析](#复杂度分析-14)
    * [完成「力扣」第 188 题：买卖股票的最佳时机 IV](#完成力扣第-188-题买卖股票的最佳时机-iv)
      * [算法思路](#算法思路-15)
      * [代码实现](#代码实现-15)
      * [复杂度分析](#复杂度分析-15)
    ...
    * [完成「力扣」第 213 题：打家劫舍 II](#完成力扣第-213-题打家劫舍-ii)
      ...
      * [代码实现](#代码实现-18)
      * [复杂度分析](#复杂度分析-18)
    * [完成「力扣」第 983 题：最低票价](#完成力扣第-983-题最低票价)
      * [算法思路](#算法思路-19)
      * [代码实现](#代码实现-19)
      * [复杂度分析](#复杂度分析-19)
  * [最长上升子序列（线性 DP 问题选讲）](#最长上升子序列线性-dp-问题选讲)
    * [「力扣」第 300 题：最长上升子序列](#力扣第-300-题最长上升子序列)
      * [算法思路](#算法思路-20)
      * [代码实现](#代码实现-20)
      * [复杂度分析](#复杂度分析-20)
<!-- TOC -->
...
### 完成「力扣」第 63 题：[不同路径 II](https://leetcode.cn/problems/unique-paths-ii)

#### 算法思路

* 网格大小为 $m \times n$，`1` 表示障碍，`0` 表示可走。
* 机器人从 `(0,0)` 出发到 `(m-1,n-1)`。
* 每步只能向右或向下。
* 任何路径只要经过障碍格就非法。

设状态 $f(i,j)$ 表示到达格子 $(i,j)$ 的不同路径数。
...
这就是无后效性的体现： $f(i,j)$ 只依赖两个相邻子问题的最优（这里是计数）结果，不依赖“怎么走来的历史过程”。

**空间压缩到一维（最快常用写法）**

用一维数组 `dp[j]` 表示“当前处理到第 $i$ 行时，到达 $(i,j)$ 的路径数”。

当我们从左到右扫描一行时：

* `dp[j]` 在更新前代表 $f(i-1,j)$（上方）
* `dp[j-1]` 已经更新过，代表 $f(i,j-1)$（左方）

因此转移为：

* 若障碍：`dp[j]=0`
* 否则：`dp[j]+=dp[j-1]`（当 $j>0$）

**核心知识点与技巧**

* 边界初始化很关键：起点如果是障碍，答案直接是 $0$。
* 第一列没有“左方”，第一行没有“上方”，但用一维 `dp` 时可以自然处理：
    * 第一列只会保留“从上往下”的累积，一旦遇到障碍就变为 $0$ 并且下面也可能继续为 $0$（除非绕路，但第一列无法绕路）。
    * 第一行只会从左往右累积，遇障碍后右侧都变 $0$。
* 因为题目保证答案 $\le 2 \times 10^9$，用 `int` 安全，无需 `long`。
...
* 时间复杂度：双重循环会把网格里每个单元格都更新一次（每格只做常数次判断/加法），因此为 $\mathcal{O}(mn)$（其中 $m$ 是行数， $n$ 是列数）。
* 空间复杂度：只使用一个长度为 $n$ 的一维数组 `dp` 作为滚动数组保存当前行状态，因此为 $\mathcal{O}(n)$（不计输入矩阵占用）。

### 完成「力扣」第 64 题：[最小路径和](https://leetcode.cn/problems/minimum-path-sum)

#### 算法思路

这题的核心是典型的网格动态规划：每个格子只能从“上方”或“左方”走到，因此到达某格的最小代价只依赖两个相邻子问题。

给定非负整数网格 $grid$，从左上角 $(0,0)$ 走到右下角 $(m-1,n-1)$，每步只能向右或向下，目标是让路径数字和最小。由于只能右/下移动，路径不会回头，天然满足“最优子结构”，适合用 **DP** 一次扫完。

题目规模是 $1 \le m,n \le 200$，最多 $4\times 10^4$ 个格子，线性扫描完全没压力。

**状态定义与转移**

设 $f(i,j)$ 表示走到格子 $(i,j)$ 的最小路径和，那么：
...
$$f(i,j) = \min\bigl(f(i-1,j),\ f(i,j-1)\bigr) + grid[i][j]$$

边界：

* 第一行只能一直向右： $f(0,j)=f(0,j-1)+grid[j]$
* 第一列只能一直向下： $f(i,0)=f(i-1,0)+grid[i]$

**空间优化技巧**

用一维数组 $dp[j]$ 表示“当前行扫到列 $j$ 时”的最小路径和：

* 扫描行 $i$ 时，更新 $dp[j]$
* 更新前的 $dp[j]$ 仍是上一行的 $f(i-1,j)$
* 更新后的 $dp[j-1]$ 是当前行的 $f(i,j-1)$

因此对内层循环有：

$$dp[j] = \min\bigl(dp[j],\ dp[j-1]\bigr) + grid[i][j]$$
...
### 完成「力扣」第 121 题：[买卖股票的最佳时机](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock)

#### 算法思路

给你一个数组 `prices`，`prices[i]` 表示第 `i` 天股价。你只能做 **一笔** 交易：选某一天买入，再选未来某一天卖出，要求买在前、卖在后，计算最大利润；如果怎么做都不赚钱，返回 `0`。

形式化一点：在所有满足 $0 \le i < j < n$ 的组合里，最大化 $prices[j]-prices[i]$。由于 $n \le 10^5$，不能用两重循环枚举（会超时）。

**一次扫描**

关键观察：如果你决定在第 `j` 天卖出，那么最佳买入价一定是前面天数 $[0..j-1]$ 里出现过的最小价格。

所以我们从左到右扫描，维护两个量：

* `minPrice`：到当前为止见过的最低价格（相当于“最划算的买入点”）
* `best`：到当前为止的最大利润

来到某天价格 `p` 时：

* 若 `p` 低于 `minPrice`，更新 `minPrice = p`（把买入点挪到更便宜的今天）
* 否则今天不比历史最低更便宜，就把今天视作卖出日候选，利润 `p - minPrice` 去更新 `best`

这保证“先买后卖”，因为 `minPrice` 只来自当前天之前（或更早）扫描过的价格。

**核心知识与技巧**

* “前缀最小值 + 当前差值”的套路：把全局最优化拆成“固定卖出日的局部最优”。
* 贪心的正确性直觉：对任意卖出日，只需要关心之前最低价，其他更贵的买入日不可能更优。
* 分支写法的小优化：当 `p < minPrice` 时直接更新，不做差值计算；在某些数据分布下能减少无效运算，但核心仍是同一个 $\mathcal{O}(n)$ 解法。

*注：为了不让为了学而学忽略了看题快速判别用什么最好的能力就没有意义了，因此本题并未直接用动态规划而是贪心*
...
* 时间复杂度：只遍历一次数组，为 $\mathcal{O}(n)$
* 空间复杂度：只用常数额外变量，为 $\mathcal{O}(1)$

### 完成「力扣」第 122 题：[买卖股票的最佳时机 II](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii)

#### 算法思路

这题允许做任意多笔交易（但同一时刻最多持有一股），最大利润的最快做法就是把所有“相邻两天的上涨”都吃到，也就是把每一段上升趋势拆成多次小交易，最终收益不变且实现最简。

你可以在任意天买入/卖出，但约束是“同时最多持有一股”，等价于：你可以完成多次“先买后卖”的交易序列，只要不重叠持仓。

数据规模是 $1 \le n \le 3 \times 10^4$，且 $0 \le prices[i] \le 10^4$，需要线性扫描解法。

**相邻涨幅累加**

核心结论：只要今天比昨天贵，就把这段涨幅加入答案。原因是，一段连续上涨从低点买到高点的总利润，等于把每天的上涨利润拆开相加：

设某段上升为 $a_0 < a_1 < \cdots < a_k$，则

$(a_k-a_0)=\sum\limits_{t=1}^{k}(a_t-a_{t-1})$

因此最大利润就是

* 遍历 $i=1..n-1$
* 若 `prices[i] > prices[i-1]`，就加上 `prices[i] - prices[i-1]`

用示例 `[7,1,5,3,6,4]` 看：相邻差为 `-6,+4,-2,+3,-2`，只加正的得到 `4+3=7`。

**核心知识点与技巧**

* 这是典型的 **贪心**：见涨就收，不猜顶不猜底。
* “同一天可多次买卖”在这里的作用是：保证把一段上升拆成多次交易在规则上完全合法（卖出后可立刻再买入），从而让“相邻涨幅累加”成立。
* 不需要显式找波谷/波峰，相邻差分更短更快，也更不容易写错边界。
...
### 完成「力扣」第 188 题：[买卖股票的最佳时机 IV](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iv)

#### 算法思路

**交易次数限制下的最优策略**

这道题是股票买卖系列中最通用、最考验动态规划理解的版本。相比于“只能买卖一次”或“无限次买卖”，这里多了一个硬性约束：**最多进行 $k$ 次交易**。

我们需要在每一天结束后，清楚地知道自己处于“第几次交易”以及“持有还是空仓”的状态。如果不记录这些状态，我们无法判断是否触达了 $k$ 次的上限。

**核心难点与极致优化**

1.  **状态存储与缓存亲和性**：
    单纯记录天数是不够的。为了追求极致的执行效率，我们摒弃了两个独立数组的写法，转而采用**内存连续的二维数组** `int[k+1][2]`。这种布局让同一笔交易的“买”与“卖”状态在内存中紧邻，极大地提升了 CPU L1 缓存的命中率（Spatial Locality）。

2.  **贪心剪枝 (Fast Path)**：
    当 $k$ 非常大时（例如 $k \ge \frac{n}{2}$），意味着交易次数不再受限，问题退化为“无限次交易”（LeetCode 122）。此时直接使用贪心算法，将时间复杂度从 $\mathcal{O}(nk)$ 降维打击至线性 $\mathcal{O}(n)$，这是冲击 0ms/1ms 的关键分水岭。
...
为了保证状态转移的物理时序性（即第 $j$ 次的“买入”必须基于第 $j-1$ 次“卖出”后的资金），我们采用**倒序遍历**交易次数 $j$。这种无分支的连续计算完美契合 CPU 的流水线机制：

1.  **卖出决策**：
    要么保持昨天的空仓状态，要么将手中的股票按当前价格 $p$ 卖出。
...
### 完成「力扣」第 983 题：[最低票价](https://leetcode.cn/problems/minimum-cost-for-tickets)

#### 算法思路

这道题是经典的序列型动态规划（Sequence DP）问题。给定一系列需要出行的日期数组 `days` 和三种不同有效期的通行证价格 `costs`，
要求以最小的成本覆盖所有的出行日。

初看问题，最直观的思路是创建一个大小为 $366$ 的布尔数组标记出行日，然后逐天递推。但如果把时间尺度拉长，或者出行的天数非常稀疏，
逐天递推会产生大量无意义的空白计算。为了追求极致的性能，我们应当剥离掉“没有出行的日子”，**直接以出行的天数索引作为 DP 的阶段**进行状态转移。

我们将状态定义在 `days` 数组的索引上：\
设 $dp[i]$ 表示**覆盖前 $i$ 个旅行日**（即覆盖 `days[0]` 到 `days[i-1]`）所需的最低总消费。

对于第 $i$ 个需要旅行的日子（对应日期为 $d = \text{days}[i-1]$），我们有三种买票策略，这三种策略由于有效期的不同，可以往前覆盖不同数量的旅行日：

1. **买单日票**：花费 $\text{costs}$，它只能覆盖今天 $d$。所以之前的旅行日必须已经被覆盖，状态从 $dp[i-1]$ 转移而来。
2. **买七日票**：花费 $\text{costs}$，它能覆盖 $[d-6, d]$ 这 $7$ 天的范围。我们需要找到在 $d-6$ 
                之前（即满足日期 $\le d-7$）的最后一个旅行日，或者换句话说，找到**第一个无法被这套七日票覆盖**的旅行日的推进索引。
3. **买三十日票**：花费 $\text{costs}$，覆盖 $[d-29, d]$。同理，我们需要找到无法被覆盖的索引位置。

由于 `days` 数组严格递增，当我们向后遍历考察 $d$ 时，那些“无法被覆盖”的临界点（索引）也必然是**单调递增**的。因此，
我们可以引入两个单调指针 $p_7$ 和 $p_{30}$。在整个 DP 遍历过程中，这两个指针只进不退，这也就是经典的**滑动窗口/双指针优化**。

状态转移方程为：

$$dp[i] = \min \begin{cases} dp[i-1] + \text{costs} \\ dp[p_7] + \text{costs} \\ dp[p_{30}] + \text{costs} \end{cases}$$

**核心知识点与技巧**

* **序列 DP (Sequence DP)**：以“完成的事件数量”而非“经历的时间”作为 DP 的阶段，能够无视时间轴的稀疏性。
* **单调指针/滑动窗口**：在需要寻找历史某个满足特定距离边界的节点时，由于目标序列的单调性，指针不需要回退，从而将内层查找的平摊复杂度降至 $\mathcal{O}(1)$。
* **错位索引 (1-based DP Array)**：将 DP 数组设为长度 $N+1$，并将起点设为 $dp = 0$。这种“多开一位”的做法能极其优雅地处理边界情况
                                （例如刚开始买的 $30$ 天票覆盖了最初的几天），避免了冗长的 `if-else` 下标越界特判。
...
* **时间复杂度**： $\mathcal{O}(N)$，其中 $N$ 为 `days` 数组的长度。外层有一个长度为 $N$ 的 `for` 循环；内层的 `while`
                 循环推进 $p_7$ 和 $p_{30}$，因为指针永不回退，它们在整个算法生命周期中最多只递增 $N$ 次。因此均摊到每次 `for` 
                 循环的内层时间是常数，总时间复杂度为严格的 $\mathcal{O}(N)$。
* **空间复杂度**： $\mathcal{O}(N)$。我们分配了一个大小为 $N+1$ 的 `dp` 数组用于储存状态。内存布局连续，对 CPU 缓存极为友好。

## 最长上升子序列（线性 DP 问题选讲）

### 「力扣」第 300 题：[最长上升子序列](https://leetcode.cn/problems/longest-increasing-subsequence/)

#### 算法思路

最长递增子序列 (Longest Increasing Subsequence, 简称 LIS) 是计算机科学中极其经典的问题。题目要求在允许跳跃元素的前提下，
找出一个尽可能长的、严格递增的序列。虽然基础的动态规划解法足以应对绝大多数普通场景，但题目进阶要求中点名的 $\mathcal{O}(N \log N)$ 
则是将算法的性能极限推向了新高度。面对最大可达 $2500$（甚至在更广泛的竞赛中达到 $10^5$）的数据约束，我们需要摒弃 $\mathcal{O}(N^2)$ 的二维比较，
采用**贪心策略与二分查找**的降维打击。

想要让一个递增子序列尽可能长，核心策略非常直白：**序列的上升速度应当尽可能慢**。或者说，对于同样长度的两个递增子序列，
我们永远更倾向于保留末尾元素较小的那一个，因为它“接纳”后续新元素的潜力更大。

基于这个贪心直觉，我们可以维护一个原生数组 `tails`，其中：\
`tails[i]` 表示在所有长度为 $i+1$ 的严格递增子序列中，**末尾元素最小的那个值**。

令人拍案叫绝的数学性质在于：**`tails` 数组必然是严格递增的**。\
因为如果一个序列长度更长，那么它的末尾元素必然大于较短序列的末尾元素。这个天然的单调递增性，完美契合了**二分查找**的先决条件。

当我们遍历原始数组 `nums` 中的每一个元素 `num` 时，我们在当前的 `tails` 数组中寻找它的归宿：

* 如果 `num` 大于 `tails` 中所有的元素，说明当前的 LIS 可以再延长一位，我们将 `num` 追加在 `tails` 的末尾。
* 如果 `num` 小于或等于 `tails` 中的某些元素，我们在 `tails` 中找到**第一个大于等于** `num` 的元素，并用 `num` 替换掉它。
* 这一步虽然不会改变当前 LIS 的长度，但它“减缓”了某个长度子序列的增长幅度，为未来的扩张埋下了伏笔。

**核心知识点与技巧**

* **Patience Sorting (耐心排序算法)**：这种贪心与二分的结合在本质上对应着计算机科学中著名的纸牌游戏排序模型。每次放置扑克牌时，
                                     要么盖住比自己大的堆的牌顶，要么在最右边新开一堆。
* **位运算二分查找**：在二分查找的区间折半步骤中，严禁使用 `(i + j) / 2`，这在处理海量数据时有整型溢出的风险。
                    必须使用无符号右移 `(i + j) >>> 1`，这是底层工程实现的安全标配。
* **连续内存状态转移**：整个算法的状态维护无需借助任何复杂的封装类或对象结构，仅靠一个原生 `int[]` 即可完成高密度的缓存命中。
...
* **时间复杂度**： $\mathcal{O}(N \log N)$。外部的 `for` 循环严格遍历了一次长度为 $N$ 的输入数组；对于其中的每一个元素，
                 在有效长度为 $K$ 的 `tails` 数组中执行了一次二分查找，二分查找的代价为 $\mathcal{O}(\log K)$。总时间复杂度受此限制。
* **空间复杂度**： $\mathcal{O}(N)$。我们分配了一个与输入数组等大的原生整型数组 `tails` 来进行状态记忆。在这个限制下已经是最优空间了，
                 且内存分配具有高度连续性，消除了一切对象冗余。
